...
def setup_saving_and_logging(config):
    ...
    arguments of config.writer and config.[trainer|inferencer], respectfully.
    Handles both training and inference configurations.
    ...
    mode_config = config.get('trainer', config.get('inferencer'))
    if mode_config is None:
        raise ValueError("Neither 'trainer' nor 'inferencer' found in config")

    save_dir = ROOT_PATH / mode_config.save_dir / config.writer.run_name
    saving_init(save_dir, config)

    # Update saving_init to handle inference mode
    if hasattr(mode_config, 'resume_from') and mode_config.resume_from is not None:
        setup_logging(save_dir, append=True)
    else:
        setup_logging(save_dir, append=False)

    logger = logging.getLogger("train" if hasattr(config, 'trainer') else "inference")
...
def saving_init(save_dir, config):
    ...
    if save_dir.exists():
        ...
        elif getattr(mode_config, 'override', False):
            print(f"Overriding save directory '{save_dir}'...")
            shutil.rmtree(str(save_dir))
        elif not getattr(mode_config, 'override', False):
            raise ValueError(
